...
namespace TamagotchiPokeApi.Controller
{
    public class TamagotchiContoller
    {
        ...
        public TamagotchiContoller()
        {
            ...
            especiesDisponiveis = pokemonApiService.ObterEspeciesDisponiveisAsync().Result; 
                        ...
                        while (true)
                        {
                            ...
                            switch (escolha)
                            {
                                ...
                                case 2:
                                    ...
                                    PokemonDetailsResult detalhes = pokemonApiService.ObterDetalhesDaEspecieAsync(especiesDisponiveis[indiceEspecie]).Result; // Fix: Use .Result to await the Task synchronously
                                    tamagotchiView.MostrarDetalhesDaEspecie(detalhes);
                                    break;
                                case 3:
                                    tamagotchiView.MostrarEspeciesDisponiveis(especiesDisponiveis);
                                    indiceEspecie = tamagotchiView.ObterEspecieEscolhida(especiesDisponiveis);
                                    detalhes = pokemonApiService.ObterDetalhesDaEspecieAsync(especiesDisponiveis[indiceEspecie]).Result; // Fix: Use .Result to await the Task synchronously
                                    tamagotchiView.MostrarDetalhesDaEspecie(detalhes);
